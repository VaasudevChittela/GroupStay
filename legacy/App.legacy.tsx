...
import HotelApp from './screens/hotel/HotelApp';
import GuestStayScreen from './screens/guest/GuestStayScreen';

type Screen = 'home' | 'advisor' | 'student' | 'staff' | 'login' | 'signup';
type AssignmentCountMap = Map<string, number>;

type GuestListItem = { id: string; legal_name: string; school: string; room_number?: string | null; room_type?: string | null; requested_roommate_id?: string | null };

type ThreadType = { id: string; name: string; canPost: boolean };
type MessageItem = { id: string; sender_name: string; sender_role: string; content: string; created_at: string };

type RoomOption = { id: string; room_number: string; room_type: string; capacity: number; school: string; occupants?: number };
type GuestInfo = { id: string; legal_name: string; school: string; arrival_window: string; is_chaperone: boolean; roommate_request_id?: string | null };
type AssignmentItem = { guest_id: string; room_id: string };
type RequestGuest = { id: string; legal_name: string; school: string; roommate_request_id: string | null };

type TripInfo = { id: string; name: string; hotel_name: string; trip_code?: string };
...
function AdvisorScreen({ onBack }: { onBack: () => void }) {
  ...
  const [currentTrip, setCurrentTrip] = useState<{ id: string; name: string; hotel_name: string; trip_code: string } | null>(null);
  ...
  const loadStats = async (tripId: string) => {
...
function AdvisorGuestListScreen({ onBack, tripId }: { onBack: () => void; tripId: string | null }) {
...
function AdvisorAssignmentScreen({ onBack, tripId }: { onBack: () => void; tripId: string | null }) {
  const [tripCode, setTripCode] = useState('');
  const [loading, setLoading] = useState(false);
  ...
  const [unassignedGuestIds, setUnassignedGuestIds] = useState<string[]>([]);

  const calculateUnassigned = (updatedAssignments: Array<{ guest_id: string; room_id: string }>, guestRows: Array<{ id: string }>) => {
  ...
  const assignmentMap = new Map<string, Array<{ id: string; legal_name: string }>>();
...
function MyRoomScreen({
  ...
  if (showKey) {
    return <GuestStayScreen guestId={guestId} trip={trip} onBack={() => setShowKey(false)} />;
  }
        ...
        <ScrollView style={styles.scrollView} contentContainerStyle={[styles.scrollContent, styles.pageContent]}>
          ...
          {loading ? (
            ...
            <>
              ...
              <TouchableOpacity style={[styles.button, { marginTop: 24 }]} onPress={() => setShowKey(true)}>
                <Text style={styles.buttonText}>Open Room Key & Wallet Pass</Text>
              </TouchableOpacity>
            </>
          ) : (
            <>
...
function RoommateRequestScreen({ onBack, guestId, tripId }: { onBack: () => void; guestId: string; tripId: string }) {
  ...
  const handleRequest = async (targetGuestId: string) => {
    setUpdating(true);
    const { error } = await supabase
      .from('guests')
  ...
  const getStatus = (guest: { id: string; roommate_request_id: string | null }) => {
...
function MessagesScreen({ onBack, role, tripId, guestId }: { onBack: () => void; role: 'advisor' | 'student'; tripId: string; guestId?: string }) {
...
function StudentScreen({ onBack }: { onBack: () => void }) {
  const [tripCode, setTripCode] = useState('');
  const [loading, setLoading] = useState(false);
  const [joinedTrip, setJoinedTrip] = useState<{ id: string; name: string; hotel_name: string } | null>(null);
  ...
  const [submittedTrip, setSubmittedTrip] = useState<{ id: string; name: string; hotel_name: string } | null>(null);
  ...
  if (showStay && guestId && submittedTrip) {
    return <GuestStayScreen guestId={guestId} trip={submittedTrip} onBack={() => setShowStay(false)} />;
  }
  ...
  if (submissionComplete) {
    return (
      <View style={styles.container}>
        ...
        <TouchableOpacity style={styles.button} onPress={() => setShowStay(true)}>
          <Text style={styles.buttonText}>My Room Key</Text>
        </TouchableOpacity>
...
function StaffScreen({ onBack }: { onBack: () => void }) {
  ...
  const [tripCode, setTripCode] = useState('');
  const [loading, setLoading] = useState(false);
  const [tripInfo, setTripInfo] = useState<{ id: string; name: string; hotel_name: string; trip_code: string } | null>(null);
  ...
  // Once a trip is loaded, hand off to the full hotel management app.
  if (tripInfo) {
    return <HotelApp trip={tripInfo} onExit={() => setTripInfo(null)} />;
  }
    ...
    <KeyboardAvoidingView style={styles.keyboardAvoidingContainer} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={90}>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <ScrollView style={styles.scrollView} contentContainerStyle={[styles.scrollContent, styles.pageContent]}>
          ...
          <Text style={styles.title}>Hotel Dashboard</Text>
          <Text style={styles.subtitle}>Enter a group's trip code to manage their rooms, guests, and keys.</Text>
          <TextInput
            style={styles.input}
            placeholder="Trip Code"
            placeholderTextColor="#94A3B8"
          ...
          <TouchableOpacity style={styles.button} onPress={loadTripDashboard} disabled={loading}>
            <Text style={styles.buttonText}>{loading ? 'Loading...' : 'Open Property Dashboard'}</Text>
          </TouchableOpacity>
...
// SafeAreaProvider must sit above every screen that uses SafeAreaView
// (the hotel dashboard and the guest key screen do).
